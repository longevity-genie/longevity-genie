...
def prepare_coronary(locations: Locations):
    ...
    source = (pl.lit("https://www.ncbi.nlm.nih.gov/snp/") + pl.col("rsID")).alias("source")
    text_col: pl.Expr = (
            ...
            + pl.lit("The study was published with pubmed id") + source
    ).alias("text")
    for_index = df.select([
                           pl.col("rsID"),
                           pl.col("Gene"),
                           pl.col("Genotype"),
                           pl.col("Population"),
                           pl.col("PMID").cast(pl.Utf8).alias("pubmed"),
                           source,
                           text_col])
    for_index.write_csv(locations.longevity_map_text, sep="\t")
    print("written to locations.longevity_map_text")
